...
import type { StageInstanceDto, StageStatus, GateDecision, GateStatus, DecideGateRequest } from "@design-platform/shared";
...
/** Steps 组件对应的阶段状态映射 */
function getStepStatus(status: StageStatus): "wait" | "process" | "finish" | "error" {
  if (status === "approved" || status === "closed") return "finish";
  if (status === "cancelled" || status === "suspended") return "error";
  if (status === "active" || status === "under_review" || status === "review_preparing" || status === "conditionally_approved") return "process";
  return "wait";
}
...
export default function StageGatePage({
  ...
  const completedStages = stages.filter((s) => s.status === "approved" || s.status === "closed").length;
  const totalStages = stages.length;
  const progress = totalStages > 0 ? Math.round((completedStages / totalStages) * 100) : 0;

  const approvedGates = gates?.filter((g) => g.decision === "approved").length ?? 0;
  const pendingGates = gates?.filter((g) => g.status === "pending").length ?? 0;
  const rejectedGates = gates?.filter((g) => g.decision === "rework_required").length ?? 0;
  ...
  const handleGateDecision = async (gateId: string, payload: DecideGateRequest) => {
    await decideGate.mutateAsync({ gateId, payload });
    setActiveGateId(null);
  };

  const canTransition = currentStage &&
    (currentStage.status === "approved" || currentStage.status === "conditionally_approved");
      ...
      <Card>
        ...
        <Steps
          ...
          items={stages.map((stage) => ({
            ...
            description: (
              <Tag color={STAGE_STATUS_COLOR[stage.status]} style={{ fontSize: 10 }}>
      ...
      {currentStage && (
        <Card>
          ...
          >
            ...
            <Title level={5} style={{ margin: 0, color: "#2563eb" }}>
              Current Stage: {STAGE_NAME_MAP[currentStage.stageCode] || currentStage.stageName}
            </Title>
          ...
          <Row gutter={16}>
            <Col span={8}>
              <div style={{ flex: 1, minWidth: 200, padding: 16, backgroundColor: "#f8fafc", borderRadius: 8 }}>
                <Text style={{ fontSize: 12, color: "#64748b", display: "block", marginBottom: 8 }}>
                  Approval Rate
                </Text>
                <div style={{ fontSize: 32, fontWeight: 700, color: "#2563eb" }}>
                  {(gates?.length ?? 0) > 0 ? Math.round((approvedGates / (gates?.length ?? 0)) * 100) : 0}%
                </div>
                <Progress
                  percent={(gates?.length ?? 0) > 0 ? Math.round((approvedGates / (gates?.length ?? 0)) * 100) : 0}
                  strokeColor="#2563eb"
                  size="small"
                  style={{ marginTop: 8 }}
                />
            ...
            <Col span={8}>
              <div style={{ flex: 1, minWidth: 200, padding: 16, backgroundColor: "#fef3c7", borderRadius: 8 }}>
                <Text style={{ fontSize: 12, color: "#64748b", display: "block", marginBottom: 8 }}>
                  Pending Items
                </Text>
            ...
            <Col span={8}>
              <div style={{ flex: 1, minWidth: 200, padding: 16, backgroundColor: "#fee2e2", borderRadius: 8 }}>
                <Text style={{ fontSize: 12, color: "#64748b", display: "block", marginBottom: 8 }}>
                  Rejected Items
                </Text>
      ...
      <Card>
        ...
        {gatesLoading ? (
          ...
          <Collapse
            ...
            onChange={(key) => setActiveGateId(typeof key === "string" ? key : null)}
              ...
              label: (
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", paddingRight: 8 }}>
                  <div>
                    ...
                    <Text type="secondary" style={{ marginLeft: 8 }}>{gate.gateCode}</Text>
                  </div>
                  <Tag color={gate.decision ? GATE_DECISION_COLOR[gate.decision] : "default"}>
                    {gate.decision ? GATE_DECISION_LABEL[gate.decision] : GATE_STATUS_LABEL[gate.status]}
                  </Tag>
                </div>
              ),
              children: (
                <div>
                  ...
                  {gate.decision && (
                    <div style={{ marginBottom: 16 }}>
                      <div style={{ display: "flex", gap: 24, fontSize: 12, color: "#64748b", flexWrap: "wrap" }}>
                        ...
                        {gate.decidedBy && <span>审批人: {gate.decidedBy}</span>}
                        {gate.decidedAt && <span>审批时间: {new Date(gate.decidedAt).toLocaleString()}</span>}
                      </div>
                      {gate.comment && (
                        <div style={{ marginTop: 8, padding: 12, backgroundColor: "#f8fafc", borderRadius: 4 }}>
                  ...
                  {gate.status === "decided" && (
                    <div style={{ textAlign: "center", padding: "8px 0", color: "#94a3b8", fontSize: 13 }}>
                      该门禁已完成决策
                    </div>
                  )}
                </div>
