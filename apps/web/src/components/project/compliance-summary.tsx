...
import { Card, Row, Col, Tag, Progress, Typography, Empty, Space } from "antd";
import {
  CheckCircleOutlined,
  CloseCircleOutlined,
...
/** 检查状态配置 */
const CHECK_STATUS_CONFIG: Record<string, { color: string; label: string; icon: React.ElementType }> = {
...
export function ComplianceSummary({ data, loading }: ComplianceSummaryProps) {
  ...
  const passRate = data.totalRules > 0
    ? Math.round((data.passedRules / data.totalRules) * 100)
    : 0;
  const statusConfig = CHECK_STATUS_CONFIG[data.checkStatus] ?? CHECK_STATUS_CONFIG["completed"]!;
          ...
          <Space direction="vertical" size={8} style={{ width: "100%" }}>
            ...
            <div style={{ display: "flex", gap: 24 }}>
              <div>
                ...
                <Text strong style={{ fontSize: 20 }}>{data.totalRules}</Text>
              </div>
              <div>
                <div style={{ fontSize: 12, color: "#64748b" }}>通过</div>
                <Text strong style={{ fontSize: 20, color: "#52c41a" }}>{data.passedRules}</Text>
              </div>
              <div>
                <div style={{ fontSize: 12, color: "#64748b" }}>失败</div>
                <Text strong style={{ fontSize: 20, color: "#ff4d4f" }}>{data.failedRules}</Text>
              </div>
            </div>
            {data.failedRules > 0 && (
              <div style={{ fontSize: 12, color: "#ff4d4f" }}>
                <CloseCircleOutlined /> 存在 {data.failedRules} 项未通过规则，需关注
              </div>
            )}
          </Space>
        </Col>
