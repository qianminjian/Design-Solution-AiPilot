...
项目规则位于 `.trae/rules/`，共 17 个规则文件：6 个始终生效、10 个按文件路径匹配、1 个按场景匹配。
...
| `remote-verification.md` | 远程验证环境执行规则、所有验证类专题在远程服务器实施     |
...
**核心策略**：本项目已部署远程验证环境，所有验证类专题统一在远程服务器实施。

- ✅ 远程环境包含 7 个服务：postgres、minio、chromadb、core、ai、bff、web + Nginx 反向代理
- ✅ 远程资源充足，不限制内存，不需要内存熔断
- ✅ 远程任务可并行 3-5 个子 agent 执行
- ❌ 禁止在本机起 docker compose 容器进行验证（本地仅用于代码编辑、单元测试、lint、typecheck）

详细规则见 `.trae/rules/remote-verification.md`。

## ⚠️ 硬禁令 内存限制（仅限本地源码加载场景）

**核心风险**：本机只有 16GB 内存，本地并行启动多个子 agent 加载大量源码时可能触发 macOS `vm-compressor-space-shortage`。

**适用范围调整（2026-08-01）**：

- 本限制**仅适用于本地子 agent 并行加载源码场景**。
- 远程验证任务**不受本限制约束**（远程资源充足）。

**本地并行加载的限制要求：**

- ❌ 禁止本地并行启动大于 6 个 subagent 同时进行大批量源码加载
- ❌ 禁止本地并行启动大于 3 个 subagent 对参考项目源代码进行加载学习，同时禁止一次性 Read 参考项目代码的整个框架
- ❌ 禁止在本地已使用内存接近 90% 时，还大批量启动 subagent
