...
**核心风险**：本机只有16G 大量大事物并发运行，触发 macOS `vm-compressor-space-shortage` → 系统强制重启。

**禁止：并行加载的限制要求：**

- ❌ 禁止并行启动大于6个 subagent 同时进行大批量测试用例的执行
- ❌ 禁止并行启动大于3个subagent 对参考项目源代码进行加载学习，同时禁止一次性 Read 参考项目代码的整个框架
- ❌ 禁止在已使用内存接近90%时，还大批量启动subagent。
